...
public class BeanProperty implements ElPropertyValue, Property, STreeProperty {
    ...
    TableJoin inheritanceTableJoin;
    String inheritanceTableJoinPrefix;
    ...
    /**
     * Return true if this property maps to the inheritance discriminator
     * column.
     */
    ...
    public void appendSelect(DbSqlContext ctx, boolean subQuery) {
        ...
        if (aggregation != null) {
            ...
            if (inheritanceTableJoin != null) {
                ctx.pushTableAlias(ctx.getRelativePrefix(inheritanceTableJoinPrefix));
            }
    ...
    /**
     * Return the DB literal predicate used to filter out soft deleted rows from
     * a query.
     */
    ...
    /**
     * Set the soft delete property value on the bean without invoking lazy
     * loading.
     */
     ...
     * is done to make any resulting Java object serialisation content smaller
     * as strings get special treatment.
     * </p>
     */
    ...
    /**
     * Read the value for this property from L2 cache entry and set it to the
     * bean.
     * <p>
    ...
    /**
     * Return true if the mutable value is considered dirty. This is only used
     * for 'mutable' scalar types like hstore etc.
     */
    ...
    /**
     * Return true if this is a draftOnly property on a non-asDraft query and as
     * such this property should not be included in a sql query.
     */
     ...
     *
     * @param platformTypes Set as false when we want logical platform agnostic
     * types.
     */
    ...
    /**
     * Return true if by default this property is set to fetch eager. Lob's
     * usually default to fetch lazy.
     */
    ...
    /**
     * Return true if this is a ManyToMany with history support (on the
     * intersection table).
     */
    ...
    /**
     * Return true if this is the tenantId property (for multi-tenant
     * partitioning).
     */
    ...
    /**
     * Return true if this property is reset/cleared on publish (on the draft
     * bean).
     */
    ...
    /**
     * Append this property to the document store based on includeByDefault
     * setting.
     */

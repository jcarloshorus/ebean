...
class SqlTreeNodeBean implements SqlTreeNode {
    ...
    public void appendSelect(DbSqlContext ctx, boolean subQuery) {
        ...
        if (readId) {
            if (!subQuery && inheritInfo != null && inheritInfo.getInheritanceType() == InheritanceType.SINGLE_TABLE) {
    ...
    public void appendWhere(DbSqlContext ctx) {
        ...
        if (inheritInfo != null && nodeBeanProp == null) {
            String where = inheritInfo.getWhere();
            if (!inheritInfo.isRoot() && where != null) {
